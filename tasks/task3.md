...
- 버그를 발생 시킨 커밋 : 25ede83
25ede83 - Doug McInnes, 10 years ago : a couple missing ends with the ipad version

- 버그 분석 : 이전 총알이 발사된 후, 다음 총알에 대한 딜레이를 통해서 연사를 막았는데, 해당 코드가 block 되어 발생한 버그임

- 수정 방법 : 해당 커밋을 revert 명령을 통해 되돌린다.
 git revert 25ede83
